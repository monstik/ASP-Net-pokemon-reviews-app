...
public class CategoryController : Controller
{
    ...
    public IActionResult GetPokemonsByCategoryId(int categoryId)
    {
        ...
        var pokemonsByCategoryId = _mapper.Map<List<PokemonDto>>(
            _categoryRepository.GetPokemonByCategory(categoryId));

        if (!ModelState.IsValid)
            return BadRequest(ModelState);

        return Ok(pokemonsByCategoryId);
    }
    ...
    public IActionResult RemoveCategory(int categoryId)
    {
        ...
        var categoryRemove = _categoryRepository.GetCategory(categoryId);

        if (!ModelState.IsValid)
            return BadRequest(ModelState);

        if (!_categoryRepository.DeleteCategory(categoryRemove))
        {
            ModelState.AddModelError("", "Something went wrong while removing category");
            return StatusCode(500, ModelState);
        }
